...
def fill_with_seed(dot_seed, color_fill, delay=False):
    ...
    while (stack):
        ...
        while (image_canvas.get(x, y) != COLOR_LINE_CHECK and image_canvas.get(x, y) != color_fill_check):
            image_canvas.put(color_fill, (x, y))
            x = x + 1
        ...
        while (x <= x_right):
            ...
            while (image_canvas.get(x, y) != COLOR_LINE_CHECK and image_canvas.get(x, y) != color_fill_check and x <= x_right):
                flag = True

                x = x + 1

            if (flag == True):
                if (x == x_right and image_canvas.get(x, y) != COLOR_LINE_CHECK and image_canvas.get(x, y) != color_fill_check):
                    stack.append([x, y])
                else:
                    stack.append([x - 1, y])
            ...
            while ((image_canvas.get(x, y) == COLOR_LINE_CHECK or image_canvas.get(x, y) == color_fill_check) and x < x_right):
                x = x + 1
    ...
    time_label.place(x=4, y=CV_HEIGHT - 900)
...
if __name__ == "__main__":
    ...
    back_box = Label(text="", font="-family {Consolas} -size 16", width=50, height=6, bg=BOX_COLOR)
    back_box.place(x=CV_WIDE + 4, y=130)

    dots_list_text = Label(win, text="Список точек", width=50, font="-family {Consolas} -size 16", bg=MAIN_TEXT_COLOR)
    dots_list_text.place(x=CV_WIDE + 4, y=110)

    dotslist_box = Listbox(bg="white")
    dotslist_box.configure(height=5, width=40)
    dotslist_box.configure(font="-family {Consolas} -size 14")
    dotslist_box.place(x=CV_WIDE + 55, y=150)

    # Add seed dot

    back_box = Label(text="", font="-family {Consolas} -size 16", width=50, height=2, bg=BOX_COLOR)
    back_box.place(x=CV_WIDE + 4, y=296)

    add_seed_dot_text = Label(win, text="Установить затравочный пиксель", width=50, font="-family {Consolas} -size 16", bg=MAIN_TEXT_COLOR)
    add_seed_dot_text.place(x=CV_WIDE + 4, y=285)

    x_seed_text = Label(text="x: ", font="-family {Consolas} -size 14", bg=BOX_COLOR)
    x_seed_text.place(x=CV_WIDE + 70, y=320)

    x_seed_entry = Entry(font="-family {Consolas} -size 14", width=9)
    x_seed_entry.place(x=CV_WIDE + 130, y=320)

    y_seed_text = Label(text="y: ", font="-family {Consolas} -size 14", bg=BOX_COLOR)
    y_seed_text.place(x=CV_WIDE + 330, y=320)

    y_seed_entry = Entry(font="-family {Consolas} -size 14", width=9)
    y_seed_entry.place(x=CV_WIDE + 390, y=320)

    add_seed_dot_btn = Button(win, text="Установить затравочный пиксель", font="-family {Consolas} -size 14", command=lambda: read_dot())
    add_seed_dot_btn.place(x=CV_WIDE + 4, y=353)

    # Fill figure

    back_box_filling = Label(text="", font="-family {Consolas} -size 16", width=50, height=3, bg=BOX_COLOR)
    back_box_filling.place(x=CV_WIDE + 4, y=395)

    color_text = Label(win, text="Выбрать тип закраски", width=50, font="-family {Consolas} -size 16", bg=MAIN_TEXT_COLOR)
    color_text.place(x=CV_WIDE + 4, y=395)
    ...
    draw_delay.place(x=CV_WIDE + 25, y=425)
    ...
    back_box_filling = Label(text="", font="-family {Consolas} -size 16", width=50, height=2, bg=BOX_COLOR)
    back_box_filling.place(x=CV_WIDE + 4, y=530)

    color_text = Label(win, text="Выбрать цвет заполнения", width=50, font="-family {Consolas} -size 16", bg=MAIN_TEXT_COLOR)
    color_text.place(x=CV_WIDE + 4, y=520)
    ...
    color_orange.place(x=CV_WIDE + 4, y=550)
    ...
    color_red.place(x=CV_WIDE + 160, y=550)
    ...
    color_blue.place(x=CV_WIDE + 300, y=550)
    ...
    color_green.place(x=CV_WIDE + 400, y=550)
