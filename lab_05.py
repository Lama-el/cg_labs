...
def dots_win():
    ...
    dot_x_label = Label(dot_win, text="X", bg="white", font="AvantGardeC 14", fg='black')
    ...
    dot_y_label = Label(dot_win, text="Y", bg="white", font="AvantGardeC 14", fg='black')
...
def answer_win(center, radius, diff, count_inside, count_outside, onside):
    ...
    ans_win.title("Ответ")
    ans_win['bg'] = "white"
    ans_win.geometry("680x200")
    ans_win.resizable(False, False)

    ans_label = Label(ans_win,
                      text="Окружность с центром в точке: (%.1f; %.1f) и c радиусом: %.1f\n"
                      ...
                      bg="violet", justify='left', font="AvantGardeC 14", fg='black')
    ans_label.place(x=10, y=5)
...
def add_dot():
    ...
    add_but = Button(dot_win, text="Добавить", font="AvantGardeC 14",bg = "white",
...
def change_dot():
    ...
    add_but = Button(dot_win, text="Изменить", font="AvantGardeC 14", bg = "white",
...
def undo():
    if len(actions) == 0:
        messagebox.showerror("Ошибка", "Разрешен возврат на одно действие назад.")
        return
...
# Множество точек
dots_label = Label(text="Точки", bg='white', font="AvantGardeC 14", fg='black')

# Список и блок точек
dots_list = []
dots_list_copy = []
dots_block = Listbox(bg="white")
dots_block.configure(font="AvantGardeC 14", fg='black')

# Кнопки
add = Button(text="Добавить", font="AvantGardeC 14",bg='white',
             borderwidth=0, command=lambda: add_dot())
dlt = Button(text="Удалить", font="AvantGardeC 14",bg='white',
             borderwidth=0, command=lambda: del_dot())
chg = Button(text="Изменить", font="AvantGardeC 14",bg='white',
             borderwidth=0, command=lambda: change_dot())
dlt_all = Button(text="Очистить", font="AvantGardeC 14",bg='white',
                 borderwidth=0, command=lambda: del_all_dots(0))
con = Button(text="Условие задачи", font="AvantGardeC 14",bg='white',
             borderwidth=0, command=lambda: messagebox.showinfo("Задание", TASK))
sol = Button(text="Решить задачу", font="AvantGardeC 14",bg='white',
             borderwidth=0, command=lambda: draw_solution())
und = Button(text="<-", font="AvantGardeC 14",bg='white',
